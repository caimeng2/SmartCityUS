...
# SDG 8.5
```{r}
p8_5 <- plot_ci(acs, unemp, 3, 8, smart, SDG8_5, "Umemployed rate (%)")
```

# SDG 9.c
```{r}
p9_c <- plot_ci(acs, internet, 79, 91, smart, SDG9_c, "With internet subscription (%)")
```
...
```
```{r}
ggarrange(p1_1, p3_8, p4_3, p8_5, p9_c, p10_4, p11_2, ncol = 1, nrow = 7, labels = "auto", label.x = 0.97) %>% ggsave(width=10, height=13, filename = "figure/sdg_outcome.png")
```
