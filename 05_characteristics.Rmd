...
# what characteristics do smart cities have
...
Smart cities are more racially and ethnically diverse. This is especially true for municipalities with small sizes.
...
```{r}
plot_mean(df, smart, bachelor, "Percentage of population with a bachelor’s degree or higher")
```
...
```{r}
plot_facet(df, smart, income, "Per capita income (dollar)")
```
```{r}
37556.12-34520.57
42698.93-36487.84
...
```{r}
plot_mean(df, smart, unemp, "Unemployed rate")
```

```{r}
plot_facet(df, smart, unemp, "Unemployed rate")
```

## Percentage of the population without health insurance

```{r}
plot_mean(df, smart, no_insurance, "Percentage of the population without health insurance")
```

```{r}
plot_facet(df, smart, no_insurance, "Percentage of the population without health insurance")
```
